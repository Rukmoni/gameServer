...
const debug = require('debug')('adventure-capitalist-server:service')
...
function GameService () {
  async function initGame () {
    ...
    if (!isNewGame) {
      ...
      if (businesses && businesses.length > 0) {
        debug('Calculating idle revenue...')
        businesses.map((business) => {
          if (business.manager === true) {
            ...
            debug(`Idle revenue for business ${business.businessKey} $${revenue}`)
          }
        })
      }
  ...
  async function manageOrder (businessKey, finishDateInMs) {
    ...
    if ((finishDateInMs - business.lastOrderStarted) >= initialTime) {
      const profit = util.getBusinessRevenue(initialProductivity, business.level, initialTime)
      const totalCashAmount = currentGame.totalCashAmount + profit
